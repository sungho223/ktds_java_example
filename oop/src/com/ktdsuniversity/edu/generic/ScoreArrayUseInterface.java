 ...
 */
public class ScoreArrayUseInterface<T> {

	public static interface CalcSum<T> {
		public T sum(Object[] arr, int size);
	}
	
	public static interface CalcAverage<T> {
		public double average(T sum, int size);
	}
	...
	public ScoreArrayUseInterface(String name) {
		this.name = name;
...
//			}
	 ...
	 * @return
	 */
	public T sum(CalcSum<T> calcSum) {
		this.sum = calcSum.sum(this.scoreArray, this.size);
		return this.sum;
	}
	
	public double average(CalcAverage<T> calcAverage) {
		return calcAverage.average(this.sum, this.size);
	}
